...
class Decoder(nn.Module):
    def __init__(self, aspp_channels=256, low_channels=24, c2_channels=32, c3_channels=96,
        ...
        self.proj_c3 = nn.Sequential(
            nn.Conv2d(c3_channels, mid_proj_channels, 1, bias=False),
        ...
        # First fusion: high (256) + c3_proj (64) → 320 → 256
        ...
        # Second fusion: 256 + c2_proj (64) → 320 → 256
        self.fuse_8 = nn.Sequential(
            nn.Conv2d(unified_channels + mid_proj_channels, unified_channels, 1, bias=False),
            nn.BatchNorm2d(unified_channels),
            nn.ReLU(inplace=True)
        )

        # Third fusion: 256 + low_proj (48) → 304 → 256
    ...
    def forward(self, high_level, low_level, c2, c3):
        ...
        # Projection layers
        c3_proc = self.proj_c3(c3) # [N, 64, H/16, W/16]
        c2_proc = self.proj_c2(c2) # [N, 64, H/8, W/8]
        low_proc = self.proj_low(low_level) # [N, 48, H/4, W/4]

        # First fusion H/16
        fused_16 = torch.cat([high_level, c3_proc], dim=1)   # [N, 320, H/16, W/16]
        fused_16 = self.fuse_16(fused_16)                    # [N, 256, H/16, W/16]

        # Second fusion H/8
        up_8 = F.interpolate(fused_16, size=c2.shape[2:], mode='bilinear', align_corners=False)
